...
def crawler():
    ...
    if municipios is None:
        return None
    municipio_barroso = [municipio for municipio in municipios if municipio['nome'].upper() == 'BARROSO'][0]
    candidates_ids = get_candidates_id(election_id, cargo_id, municipio_barroso)
    if candidates_ids is None:
        return None
    for candidate_id in candidates_ids:
        candidate = get_candidate(election_id, uf_sigla, municipio_barroso, candidate_id)
